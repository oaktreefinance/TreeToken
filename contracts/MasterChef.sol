...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event UpdateEmissionRate(address indexed user, uint256 treePerBlock);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 treeReward = multiplier.mul(treePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        tree.mint(devaddr, treeReward.div(10));
    ...
    function safeTreeTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safeTreeTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _treePerBlock) public onlyOwner {
        massUpdatePools();
        treePerBlock = _treePerBlock;
        emit UpdateEmissionRate(msg.sender, _treePerBlock);
    }
}
